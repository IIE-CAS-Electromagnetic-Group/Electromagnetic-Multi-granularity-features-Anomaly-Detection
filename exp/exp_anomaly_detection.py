...
class Exp_Anomaly_Detection(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        new_test_labels = np.concatenate([test_label.detach().cpu().numpy() for test_label in test_labels],axis=0)
        ...
        # result save
        folder_path = './anomaly_detection_results/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        ...
        f.close()
        return
    def anomaly_detection(self, setting, test=0,root_path=''):
        ...
        test_labels = []
        dates = []
        for i, (batch_x, batch_y, batch_date) in enumerate(test_loader):
            batch_x = batch_x.float().to(self.device)
            # reconstruction
            outputs = self.model(batch_x, None, None, None)
            # criterion
            ...
            dates.append(batch_date.detach().cpu().numpy())
        # 保存预测和真实值，标签值
        # preds = np.array(preds)
        # trues = np.array(trues)
        preds = np.concatenate(preds, axis=0)
        ...
        f.write(setting + "  \n")
        f.write(
            "Threshold: {:0.4f}, Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f} ".format(
                threshold,
                accuracy, precision,
                recall, f_score))
        f.write('\n')
